...
1. Regenerate the pinned dependencies using the helper script. This step is required, otherwise the build will fail or
   ...
   - `bash scripts/generate-nuget-deps.sh --ensure-sibling azure.mcp:azure.mcp.linux-x64 Azure.Mcp <Version> packages/azure-mcp-server/deps.json`
   ...
   them to `packages/azure-mcp-server/deps.json`.
...
- If you run the script without a version, it will resolve the latest available:
  `bash scripts/generate-nuget-deps.sh Azure.Mcp packages/azure-mcp-server/deps.json`.
